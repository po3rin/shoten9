...
プロパティには必須の項目、必須ではないが、コンポーネント中にひとつしかセットできない項目、0個以上設定できる項目があります。
...
文字列はダブルクオートで囲まれた文字列とそれ以外では使用できる文字が異なります。
ダブルクオートで処理された場合は専用の文字列判別ルールを用いて処理します。
...
最後に@<tt>{VALUE}へ変換します。
@<list>{implementation_convert_content_line}では値の変換する前にコロンの字句があるかチェックしています。
...
==== テストを書く
ここまでの変換処理で文字列をiCalendarで定義されているコンテンツラインへの変換の実装が終わりました。
...
@<tt>{Integer}型はGoの基本型にあるため、単に@<tt>{int64}型に独自の名前の型にします。
@<tt>{int64}型をそのまま使わずに型を定義したのには理由があります。
プロパティには複数種類の型を値として保持するプロパティがあります。
